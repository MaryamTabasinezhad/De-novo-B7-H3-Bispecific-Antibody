...
try:
    from coverage_scope import build_coverage_scope
    from query_profiles import all_registry_statuses
except ImportError:
    from scripts.coverage_scope import build_coverage_scope
    from scripts.query_profiles import all_registry_statuses
...
def query_trials(
    ...
    statuses : list of str or None
        Trial statuses. Default: all 14 registered-record and expanded-access statuses.
    ...
    return_metadata : bool
        If True, return ``(trials, coverage_scope)``. The coverage scope proves
        that pagination was exhausted and the retrieved count matched the API total.
    query_profile : dict or None
        Versioned query-profile metadata recorded in coverage_scope.json.

    Returns
    -------
    list of dict or tuple
        Flat trial records, or ``(records, coverage_scope)`` when
        ``return_metadata=True``. Records contain keys: nct_id, brief_title, official_title,
        ...
        is_fda_regulated_drug, has_dmc, brief_summary, detailed_description.
    ...
    Verification
    ------------
    Prints "✓ Retrieved {N} registered records from ClinicalTrials.gov"
    """
    ...
    for page_num in range(max_pages):
        ...
        try:
            ...
            print(f"ERROR: {e}")
            raise RuntimeError(
                "ClinicalTrials.gov API request failed; partial results were discarded: "
                f"{e}"
            ) from e
        ...
        if not isinstance(total_count, int):
            raise RuntimeError("ClinicalTrials.gov response omitted the integer totalCount")
        if api_total_count is not None and total_count != api_total_count:
            raise RuntimeError(
                "ClinicalTrials.gov totalCount changed during pagination: "
                f"{api_total_count} -> {total_count}"
            )
    ...
    if not pagination_exhausted:
        raise RuntimeError(
            f"ClinicalTrials.gov pagination exceeded max_pages={max_pages}; "
            "partial results were discarded. Increase max_pages and rerun the full query."
        )
    if len(all_trials) != api_total_count:
        raise RuntimeError(
            "ClinicalTrials.gov retrieved count did not match totalCount; "
            f"partial results were discarded ({len(all_trials)} != {api_total_count})."
        )

    coverage_scope = build_coverage_scope(
        ...
        records_retrieved=len(all_trials),
        api_total_count=api_total_count,
        pagination_exhausted=pagination_exhausted,
        input_resolution="versioned_profile" if query_profile else "declared_query",
        query_profile=query_profile,
    )

    print(f"\n✓ Retrieved {len(all_trials)} registered records from ClinicalTrials.gov")
    print("=" * 70 + "\n")

    if return_metadata:
        return all_trials, coverage_scope
    return all_trials
...
def _extract_trial(study):
    ...
    # Descriptions are classification and query-match evidence.
...
if __name__ == "__main__":
    ...
    if trials:
        ...
        print(f"  Detailed description: {t['detailed_description'][:120]}...")
