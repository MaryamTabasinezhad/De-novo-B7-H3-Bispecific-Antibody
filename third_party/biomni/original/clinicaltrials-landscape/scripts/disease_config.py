"""
Optional disease-configuration loader for the ClinicalTrials.gov landscape skill.

No disease config ships with this package. Classification therefore runs in the
record-grounded generic mode by default (study purpose + intervention type; see
classify_study.py). A user MAY supply their own curated YAML at
disease_configs/<disease_id>.yaml to override the generic taxonomy with a
mechanism-of-action taxonomy for a specific disease; when none is present,
load_disease_config() returns None and the generic classifier is used. This
module never fabricates a taxonomy and makes no claim that a pre-built config is
available.
"""
