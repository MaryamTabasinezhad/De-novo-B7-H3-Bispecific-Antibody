#!/usr/bin/env python3
"""Prepare RFantibody HLT framework and spatial target crops for the two approved patches."""
...
def prepare_target(src, out, core, margin=12.0):
    lines=src.read_text().splitlines()
    atoms=[l for l in lines if l[:6] in AA_ATOM and l[21].strip()=="A" and 29<=int(l[22:26])<=466]
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--target',type=Path,required=True); ap.add_argument('--framework',type=Path,required=True); ap.add_argument('--outdir',type=Path,required=True); args=ap.parse_args()
    ...
    for arm,core in specs.items():
        residues=prepare_target(args.target,args.outdir/f'target_{arm}.pdb',core)
